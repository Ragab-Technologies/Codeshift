...
# =============================================================================
# DECLARATIVE BASE (SQLAlchemy 1.4 style)
# =============================================================================
...
def setup_database() -> tuple[Any, Session]:
    ...
    engine = create_engine(
        ...
        echo=True,
        future=True,  # This flag should be removed in 2.0
    )
...
def complex_query_examples(session: Session) -> None:
    ...
    # Query with one()
    single_user = session.query(User).filter(User.id == 1).one()

    # Query with one_or_none()
    maybe_user = session.query(User).filter(User.email == "test@example.com").one_or_none()
    ...
    high_spending_users = (
        session.query(User)
        .filter(User.id.in_(session.query(high_spenders_subquery.c.user_id)))
        .all()
    )
    ...
    users_with_order_count = (
        session.query(User, order_count_subq.label("order_count"))
        .all()
    )
    ...
    articles_only = session.query(Article).all()

    # Query with polymorphic loading
    content_with_subclasses = session.query(
        with_polymorphic(Content, [Article, Video])
    ).all()
    ...
    admins = (
        session.query(User)
        .filter(User.has_role(UserRole.ADMIN))
        .all()
    )
