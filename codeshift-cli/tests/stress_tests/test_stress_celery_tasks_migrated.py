...
from celery import Celery, Task, chain, chord, group, shared_task
...
app.conf.imports = ['tasks.email', 'tasks.reports']
app.conf.include = ['tasks.notifications']
...
app.conf.worker_task_time_limit = 7200
app.conf.worker_task_soft_time_limit = 6000
...
@shared_task(bind=True, name='tasks.multiply')
...
@shared_task(queue='priority', routing_key='priority.high')
...
@shared_task(bind=True, on_failure=on_task_error)
