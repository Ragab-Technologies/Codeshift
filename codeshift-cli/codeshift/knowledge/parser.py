...
NOTE: This module uses the internal _LLMClient for knowledge base generation,
which is an internal/admin operation. This is intentional - knowledge base
generation runs on the Codeshift backend, not in the CLI.

For client-side LLM operations, use CodeshiftAPIClient instead.
"""
...
# Internal imports - these are private and used only for knowledge base generation
# which runs server-side, not in the CLI
...
class ChangelogParser:
    ...
    NOTE: This class is intended for internal/server-side use only.
    Knowledge base generation runs on the Codeshift backend, not in the CLI.
    """
...
If there are no breaking changes, respond with an empty array: []"""
    ...
    def __init__(self, client: _LLMClient | None = None):
        ...
        NOTE: This is for internal/server-side use only.
