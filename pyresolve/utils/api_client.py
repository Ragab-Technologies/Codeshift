...
from pyresolve.cli.commands.auth import get_api_key, get_api_url
...
class PyResolveAPIClient:
    ...
    def _make_request(
        ...
        """
        if not self.api_key:
            raise ValueError("API key not configured. Run 'pyresolve login' to authenticate.")
    ...
    def migrate_code(
        ...
        if not self.is_available:
            return APIResponse(
                success=False,
                content=code,
                error="Not authenticated. Run 'pyresolve login' to use LLM migrations.",
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return APIResponse(
                    success=False,
                    content=code,
                    error="Authentication failed. Run 'pyresolve login' to re-authenticate.",
                )
                ...
                return APIResponse(
                    ...
                    error=(
                        ...
                        f"Upgrade at {detail.get('upgrade_url', 'https://pyresolve.dev/pricing')}"
                    ),
                )
                ...
                return APIResponse(
                    success=False,
                    content=code,
                    error="LLM migrations require Pro tier or higher. Run 'pyresolve upgrade-plan' to upgrade.",
                )
    ...
    def explain_change(
        ...
        if not self.is_available:
            return APIResponse(
                success=False,
                content="",
                error="Not authenticated. Run 'pyresolve login' to use this feature.",
            )
